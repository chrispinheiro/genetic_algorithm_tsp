...
def generate_random_population(att_cities_location: List[Tuple[float, float]], population_size: int) -> List[List[Tuple[float, float]]]:
    """
    ...
    """
    return [random.sample(att_cities_location, len(att_cities_location)) for _ in range(population_size)]
