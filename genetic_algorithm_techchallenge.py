...
def generate_random_population(ordem_cidades: List[int], population_size: int) -> List[int]:
    ...
    Returns:
    List[List[int]]: A list of routes, where each route is represented as a list of city locations.
    """
...
def distancia_cidades(point1: int, point2: int) -> float:
    """
    ...
    Returns:
    float: Distance between the two points find Distance Matriz.
    """
    #return matriz_distancia[localizacao.index((point1[0],point1[1]))][localizacao.index((point2[0],point2[1]))]
    return matriz_distancia[point1][point2]
...
#         distance += distancia_cidades(path[i], path[(i + 1) % n])
...
def calculate_fitness(path: List[int]) -> float:
    ...
    Parameters:
    - path (List[Int]): A list of integer representing the path,
      where each interger contains the order of a point.
    ...
    for i in range(n):
        #distance += calculate_distance(path[i], path[(i + 1) % n])
        distance += distancia_cidades(path[i-1], path[(i)])
...
# def haversine(point1: Tuple[float, float], point2: Tuple[float, float]):
